...
def load_scores() -> pd.DataFrame:
    # ייבוא עצל — נטען רק כשנדרש, לא בעת import של המודול
    from scores_news.cat_scores.bonds_score import (
        fetch_treasury_yield_xml, extract_yields_from_treasury_xml, calculate_bond_score)
...
def load_time_series() -> pd.DataFrame:
    ...
    return pd.DataFrame(index=dates, columns=cols).fillna(50)
